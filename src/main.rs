...
fn get_args() -> Result<(u32, u32, u32, u32), &'static str> {
    ...
    let width = args.get(0).map(|x| x.parse::<u32>().ok()).flatten().unwrap_or(128);
    let height = args.get(1).map(|x| x.parse::<u32>().ok()).flatten().unwrap_or(64);
    let num_threads = args.get(2).map(|x| x.parse::<u32>().ok()).flatten().unwrap_or(4);
    let ups = args.get(3).map(|x| x.parse::<u32>().ok()).flatten().unwrap_or(15);
...
fn main() {
    ...
    event_loop.run(move |event, _, control_flow| {
        ...
        let elased = now.duration_since(previous);
        game.update(elased.as_secs_f64());
